...
int main(int argc, char **argv) {
  if (argc <= 1) {
    ...
    __builtin___strcpy_chk (file_info.file_name, temp, __builtin_object_size (file_info.file_name, 2 > 1 ? 1 : 0));
    lex_file();
    return 0;
  }
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (temp == ((void *)0)) {
      fprintf(__stderrp, "Error opening file %s", argv[i]);
      return 1;
    }
    file_info.file_line_start = 0;
    if (file_info.file_name != ((void *)0)) {
      free(file_info.file_name);
    }
    file_info.file_name =
        (char *)malloc(sizeof(char) * strnlen(argv[i], 1024));
    __builtin___strncpy_chk (file_info.file_name, argv[i], 1024, __builtin_object_size (file_info.file_name, 2 > 1 ? 1 : 0));
    yyin = temp;
    lex_file();
  }
}
